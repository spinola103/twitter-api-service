...
// Bulk endpoint for n8n (optimized for 10 accounts per hour)
app.post('/scrape/bulk', async (req, res) => {
  ...
  // Process accounts with delay to avoid hitting rate limits
  for (let i = 0; i < usernames.length; i++) {
    const username = usernames[i];
    
    try {
      // Add progressive delay to spread requests
      if (i > 0) {
...
app.listen(PORT, '0.0.0.0', () => {
  ...
  console.log(`   POST /scrape - Single account`);
  console.log(`   POST /scrape/bulk - Multiple accounts`);
